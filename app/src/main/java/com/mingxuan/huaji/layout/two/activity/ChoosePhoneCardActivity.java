...
import com.mingxuan.huaji.layout.two.model.CardOrderModel;
    ...
    private void findOrder(){
        ...
        MainApi.getInstance(this).findPhoneCardOrderApi(createId, new GetResultCallBack() {
                    ...
                    public void getResult(String result, int type) {
                        loadingDialog.dismiss();
                        if(type == Constants.TYPE_SUCCESS){
                            List<CardOrderModel.ResultBean> resultBeans = GsonUtil.fromJsonList(new Gson(),result,CardOrderModel.ResultBean.class);
                            orderlist.clear();
                            orderlist.addAll(resultBeans);

                            long time = orderlist.get(0).getCreateTime();
                            showDialog(orderlist.get(0).getInfo(),Double.parseDouble(orderlist.get(0).getPrice()),simpleDateFormat.format(new Date(time)));
                        }else BaseApi.showErrMsg(ChoosePhoneCardActivity.this,result);
                    }
                });
    }
